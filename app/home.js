...
export default function home() {
  ...
  useEffect(() => {
    // Create an event listener for app state changes
  ...
  const handleAppStateChange = (nextAppState) => {
    if (appState === 'active' && nextAppState.match(/inactive|background/)) {
      // App has gone to the background, update user status to "offline"
      updateUserStatus();
    }
    ...
    <LinearGradient colors={["#1e1f24", "#23232b"]} style={stylesheet.view1}>
      ...
      <View style={stylesheet.view4}>
        ...
        <Pressable onPress={
          ...
          <FontAwesome6 name={"users-viewfinder"} size={25} color={"white"} />
        </Pressable>
      </View>
      ...
      <View style={stylesheet.scrollView2}>
        ...
        <FlashList
          ...
          renderItem={
            ({ item }) =>
              item.other_user_status == 1 && !item.newChat ?
